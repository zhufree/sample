...
class Reservelib(SGMLParser):
    ...
    # 获取空闲研修室信息，参数为年月日和学部（1是本部，2是医学部）
        ...
        if self.region  ==  '1':
            ...
            try:
                soup_1_6 = BeautifulSoup(urllib2.urlopen(url_1_6, timeout=4), 'lxml')
                soup_1_8 = BeautifulSoup(urllib2.urlopen(url_1_8, timeout=4), 'lxml')
                soup_1_9 = BeautifulSoup(urllib2.urlopen(url_1_9, timeout=4), 'lxml')
            except Exception, e:
        ...
        # 医学部
            ...
            try:
                soup_2_14 = BeautifulSoup(urllib2.urlopen(url_2_14, timeout=4), 'lxml')
                soup_2_12 = BeautifulSoup(urllib2.urlopen(url_2_12, timeout=4), 'lxml')
            except Exception, e:
                error["error_code"] = 10003
    ...
    # 登录获得cookie字符串
        if sid and pwd:
            ...
            try:
                soup = BeautifulSoup(urllib2.urlopen(req, timeout=4), 'lxml')# 处理返回链接
            except Exception, e:
                error["error_code"] = 10003
                error["reason"] = u"网络连接错误"
                error["result"] = []
