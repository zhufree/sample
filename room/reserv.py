...
class reservlib(SGMLParser):
    def getroominfo(self,year,month,day,region):#获取空闲研修室信息，参数为年月日和学部（1是本部，2是医学部）
        self.year=year
        self.month=month
        self.day=day
        self.region=region
...
#本部
        if self.region=='1':
            ...
            try:
                soup_1_6=BeautifulSoup(urllib2.urlopen(url_1_6,timeout=4))
                soup_1_8=BeautifulSoup(urllib2.urlopen(url_1_8,timeout=4))
                soup_1_9=BeautifulSoup(urllib2.urlopen(url_1_9,timeout=4))
            except Exception,e:
                ...
                for i in s_1_6:
                    ...
                    if hour=="hour=08":
                        rooms_2['1'].append(room)    
                    elif hour=="hour=11":
                        rooms_2['2'].append(room)
                    elif hour=="hour=15":
                        rooms_2['3'].append(room)
                    elif hour=="hour=18":
                        rooms_2['4'].append(room)
            #判断空闲时间段，并加入该研修室列表中，下同
                ...
                for i in s_1_8:
                    ...
                    if room=="room=105":
                        if hour=="hour=08":
                            rooms_8['1'].append(room)
                        elif hour=="hour=11":
                            rooms_8['2'].append(room)
                        elif hour=="hour=15":
                            rooms_8['3'].append(room)
                        elif hour=="hour=18":           
                            rooms_8['4'].append(room)
                        dic['B1.8']=rooms_8
                    else:
                        if hour=="hour=08":
                            rooms_4['1'].append(room)
                        elif hour=="hour=11":    
                            rooms_4['2'].append(room)
                        elif hour=="hour=15":
                            rooms_4['3'].append(room)
                        elif hour=="hour=18":
                            rooms_4['4'].append(room)    
                        dic['B1.4']=rooms_4
                rooms_2={}
                rooms_2['1']=[]
                rooms_2['2']=[]
...
#医学部    
    ...
    def getcookie(self,sid,pwd):#登录获得cookie字符串
        if sid and pwd:
            ...
            try:
                soup=BeautifulSoup(urllib2.urlopen(req,timeout=4))#处理返回链接
            except Exception,e:
                error["error_code"]=10003
                error["reason"]=u"网络连接错误"
                error["result"]=[]
    ...
    def cancel(self):
        if hasattr(self, "roomid"):
            handpage="http://reserv.lib.whu.edu.cn/del_entry.php?id=%s"%(self.roomid)
